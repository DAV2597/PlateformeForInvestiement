...
<body>
    ...
    <script>
        function myfonction(){
            const base2donnees=JSON.parse(localStorage.getItem('rodrigue')) || [];
            affichescreen(base2donnees)
        }
        function affichescreen(base2donnees){
            const tb1=document.querySelector("#TLB tbody");
            tb1.innerHTML='';
        ...
        function V(a,p){
            ...
            localStorage.setItem('rodrigue' , JSON.stringify(base2donnees))
            myfonction()
        }
        ...
        resultat.addEventListener('submit',(e)=>{
            e.preventDefault()
             const a1=document.getElementById("h").value;
             const p1=document.getElementById("j").value;
